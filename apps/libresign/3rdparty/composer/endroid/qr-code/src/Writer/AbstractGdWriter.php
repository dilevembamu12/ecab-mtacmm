...
abstract readonly class AbstractGdWriter implements WriterInterface, ValidatingWriterInterface
{
    ...
    private function addLogo(LogoInterface $logo, GdResult $result) : GdResult
    {
        ...
        \imagecopyresampled($targetImage, $logoImageData->getImage(), \intval($matrix->getOuterSize() / 2 - $logoImageData->getWidth() / 2), \intval($matrix->getOuterSize() / 2 - $logoImageData->getHeight() / 2), 0, 0, $logoImageData->getWidth(), $logoImageData->getHeight(), \imagesx($logoImageData->getImage()), \imagesy($logoImageData->getImage()));
