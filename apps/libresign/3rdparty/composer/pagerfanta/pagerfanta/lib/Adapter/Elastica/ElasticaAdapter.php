...
class ElasticaAdapter implements AdapterInterface
{
     ...
     */
    public function __construct(private readonly SearchableInterface $searchable, private readonly Query $query, private readonly array $options = [], ?int $maxResults = null)
    {
    ...
    {
        return $this->resultSet = $this->searchable->search($this->query, \array_merge($this->options, ['from' => $offset, 'size' => $length]));
    }
}
