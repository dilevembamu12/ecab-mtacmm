...
class InfoFile extends File
{
     ...
     */
    public static $documentInfoFields = array('Title', 'Author', 'Subject', 'Keywords', 'Creator', 'Producer', 'CreationDate', 'ModDate', 'Trapped');
    /**
     * Constructor
     *
     ...
     */
    public function __construct(array|InfoFields $data, ?string $suffix = null, ?string $prefix = null, ?string $directory = null, ?string $encoding = 'UTF-8')
    {
        if ($suffix === null) {
            $suffix = '.txt';
        }
        ...
        $tempName = \tempnam($directory, $prefix);
    ...
    private static function renderBlock(string $block, array $items, string $encoding) : string
    {
        ...
        foreach ($items as $key => $value) {
            if ($block === 'Info') {
                $fields .= self::renderField($block, $key, $value, $encoding, \true);
            } else {
                $fields .= "{$block}Begin\n";
     ...
     */
    private static function renderField(string $prefix, string $key, string $value, string $encoding, bool $isInfo) : string
    {
        if ($encoding !== 'UTF-8') {
