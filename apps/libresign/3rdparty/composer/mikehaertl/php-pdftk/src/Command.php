...
class Command extends BaseCommand
{
    ...
    public function addFile($name, $handle, $password = null) : self
    {
        ...
        $file = array('name' => $name, 'password' => $password);
     ...
     * @return Command the command instance for method chaining
     */
    public function addPageRange(int|string|array $start, int|string|null $end = null, string|null $handle = null, string|null $qualifier = null, string|null $rotation = null)
    {
        $this->checkExecutionStatus();
        if (\is_array($start)) {
            if ($handle !== null) {
    ...
    protected function processOptions($filename = null) : void
    {
        ...
        if ($filename !== null) {
            $this->addArg('output', $filename, \true);
        }
