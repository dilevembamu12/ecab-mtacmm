...
namespace OCA\Libresign\Vendor\mikehaertl\shellcommand;
...
class Command
{
    ...
    public function addArg($key, $value = null, $escape = null)
    {
        ...
        if ($value === null) {
            ...
            if (\is_array($value)) {
                ...
                $this->_args[] = $argKey . $separator . ($doEscape ? \escapeshellarg($value) : $value);
            }
        }
        if ($useLocale) {
     ...
     */
    public function getOutput($trim = \true, $characters = " \t\n\r\x00\v\f")
    {
        return $trim ? \trim($this->_stdOut, $characters) : $this->_stdOut;
    }
    /**
     * @param bool $trim whether to `trim()` the return value. The default is `true`.
     * @param string $characters the list of characters to trim. The default
     ...
     */
    public function getError($trim = \true, $characters = " \t\n\r\x00\v\f")
    {
        return $trim ? \trim($this->_error, $characters) : $this->_error;
    }
    /**
     * @param bool $trim whether to `trim()` the return value. The default is `true`.
     * @param string $characters the list of characters to trim. The default
     ...
     */
    public function getStdErr($trim = \true, $characters = " \t\n\r\x00\v\f")
    {
        return $trim ? \trim($this->_stdErr, $characters) : $this->_stdErr;
    }
    /**
    ...
    public function execute()
    {
        ...
        if ($this->useExec) {
            ...
            $isInputStream = $this->_stdIn !== null && \is_resource($this->_stdIn) && \in_array(\get_resource_type($this->_stdIn), array('file', 'stream'));
            ...
            $nonBlocking = $this->nonBlockingMode === null ? !$this->getIsWindows() : $this->nonBlockingMode;
            $startTime = $hasTimeout ? \time() : 0;
            ...
            if (\is_resource($process)) {
                if ($nonBlocking) {
                    ...
                    while ($isRunning) {
                        ...
                        $runTime = $hasTimeout ? \time() - $startTime : 0;
                ...
                if ($this->_exitCode !== 0) {
                    $this->_error = $this->_stdErr ? $this->_stdErr : "Failed without error message: {$command} (Exit code: {$this->_exitCode})";
                    return \false;
                }
            } else {
