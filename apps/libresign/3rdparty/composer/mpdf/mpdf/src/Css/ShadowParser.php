...
class ShadowParser
{
    ...
    {
        $boxShadow = ['inset' => \false, 'blur' => 0, 'spread' => 0];
        ...
        if (isset($p[0])) {
            ...
            $boxShadow['x'] = $this->sizeConverter->convert(\trim($p[0]), $parentWidth, $this->mpdf->FontSize, \false);
        }
        if (isset($p[1])) {
            $parentWidth = 0;
            if (isset($this->mpdf->blk[$this->mpdf->blklvl - 1]['inner_width'])) {
            ...
            $boxShadow['y'] = $this->sizeConverter->convert(\trim($p[1]), $parentWidth, $this->mpdf->FontSize, \false);
        }
        if (isset($p[2])) {
            if (\preg_match('/^\\s*[\\.\\-0-9]/', $p[2])) {
                ...
                $boxShadow['blur'] = $this->sizeConverter->convert(\trim($p[2]), $parentWidth, $this->mpdf->FontSize, \false);
            } else {
                $boxShadow['col'] = $this->colorConverter->convert(\preg_replace('/\\*/', ',', $p[2]), $this->mpdf->PDFAXwarnings);
            }
        }
        if (isset($p[3])) {
            if (\preg_match('/^\\s*[\\.\\-0-9]/', $p[3])) {
                ...
                $boxShadow['spread'] = $this->sizeConverter->convert(\trim($p[3]), $parentWidth, $this->mpdf->FontSize, \false);
            } else {
                $boxShadow['col'] = $this->colorConverter->convert(\preg_replace('/\\*/', ',', $p[3]), $this->mpdf->PDFAXwarnings);
            }
        }
        if (isset($p[4])) {
            $boxShadow['col'] = $this->colorConverter->convert(\preg_replace('/\\*/', ',', $p[4]), $this->mpdf->PDFAXwarnings);
        }
    ...
    protected function parseSingleTextShadow($s)
    {
        ...
        $p = \explode(' ', \trim($s));
        if (isset($p[0])) {
            $textShadow['x'] = $this->sizeConverter->convert(\trim($p[0]), $this->mpdf->FontSize, $this->mpdf->FontSize, \false);
        }
        if (isset($p[1])) {
            $textShadow['y'] = $this->sizeConverter->convert(\trim($p[1]), $this->mpdf->FontSize, $this->mpdf->FontSize, \false);
        }
        if (isset($p[2])) {
            if (\preg_match('/^\\s*[\\.\\-0-9]/', $p[2])) {
                $textShadow['blur'] = $this->sizeConverter->convert(\trim($p[2]), isset($this->mpdf->blk[$this->mpdf->blklvl]['inner_width']) ? $this->mpdf->blk[$this->mpdf->blklvl]['inner_width'] : 0, $this->mpdf->FontSize, \false);
            } else {
                $textShadow['col'] = $this->colorConverter->convert(\preg_replace('/\\*/', ',', $p[2]), $this->mpdf->PDFAXwarnings);
            }
        }
        if (isset($p[3])) {
            $textShadow['col'] = $this->colorConverter->convert(\preg_replace('/\\*/', ',', $p[3]), $this->mpdf->PDFAXwarnings);
        }
        if (empty($textShadow['col'])) {
            $textShadow['col'] = $this->colorConverter->convert('#888888', $this->mpdf->PDFAXwarnings);
        }
