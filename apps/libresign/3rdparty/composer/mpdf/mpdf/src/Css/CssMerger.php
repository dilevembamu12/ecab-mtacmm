...
use OCA\Libresign\Vendor\Mpdf\Exception\InvalidArgumentException;
...
class CssMerger
{
     ...
     */
    private $sideEffects = \true;
    public function __construct(Mpdf $mpdf, NormalizeProperties $normalizeProperties, InlineStyleParser $inlineStyleParser, SelectorParser $selectorParser, InlinePropertyConverter $inlinePropertyConverter, ColorConverter $colorConverter, BorderMerger $borderMerger)
    {
        $this->mpdf = $mpdf;
    ...
    {
        if (!\in_array($inherit, ['TOPTABLE', 'TABLE'], \true) || !$this->sideEffects) {
            return;
        }
        if ($inherit === 'TOPTABLE') {
        ...
        $this->mergeFullCssRules($this->cssManager->cascadeCSS, $this->cssManager->tablecascadeCSS[$this->cssManager->tbCSSlvl], $tag, $classes, $attr['ID'], $attr['LANG']);
        // Cascading forward CSS
        if (isset($this->cssManager->tablecascadeCSS[$this->cssManager->tbCSSlvl - 1])) {
            $this->mergeFullCssRules($this->cssManager->tablecascadeCSS[$this->cssManager->tbCSSlvl - 1], $this->cssManager->tablecascadeCSS[$this->cssManager->tbCSSlvl], $tag, $classes, $attr['ID'], $attr['LANG']);
        }
    }
    /**
    ...
    protected function mergeBlockCascadingCss($inherit, $tag, $attr, $classes)
    {
        ...
        // Save Cascading CSS e.g. "div.topic p" at this block level
        $this->mergeFullCssRules($this->cssManager->cascadeCSS, $currentBlock['cascadeCSS'], $tag, $classes, $attr['ID'], $attr['LANG']);
        // Cascading forward CSS
        $this->mergeFullCssRules($previousBlock['cascadeCSS'], $currentBlock['cascadeCSS'], $tag, $classes, $attr['ID'], $attr['LANG']);
        ...
        if (isset($previousBlock['InlineProperties'])) {
            ...
            $this->cssProperties = \array_merge($this->cssProperties, $converted);
            // mPDF 5.7.1
        }
    }
    /**
    ...
    {
        if (!\in_array($tag, ['TABLE', 'TD', 'TH'], \true)) {
            return;
        }
    ...
    {
        if (!\in_array($tag, ['TR', 'TH', 'TD'], \true) || empty($sourceSelectors)) {
            return;
        }
        $regex = '/(([\\-+]?\\d*)?N([\\-+]\\d+)?|[\\-+]?\\d+|ODD|EVEN)/';
     ...
     * @return void
     */
    protected function setMergedCss(&$property, $strictMode = \true, $borderDominanceLevel = \false)
    {
        if (!isset($property)) {
            return;
        }
